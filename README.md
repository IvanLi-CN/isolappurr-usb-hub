...
- 列出 Local USB 候选串口：`just local-ports`
- 读取设备 `info` 并确认串口身份：`PORT=/dev/cu.xxx just local-identify`（写入 `.esp32-port` 与 `.esp32-port.identity.json`）
- 生成 app `.bin`：`just firmware-bin`
- 烧录 + reset + 监视：`just local-flash-monitor`
...
- Local USB 烧录 + 串口监视（推荐）：`just local-flash-monitor`
  - 由项目内 `isolapurr-desktop` CLI 执行；串口缓存为 `.esp32-port`，身份缓存为 `.esp32-port.identity.json`。
  - 烧录前会通过 JSONL `info` 校验 `device_id` / `mac`，只写 app `.bin` 到 `0x10000`，不会默认写 full image。
- `cargo run --release`（可选）：会通过 `tools/mcu-agentd-runner` 调用项目内 Local USB runner（同样要求先运行 `PORT=/dev/cu.xxx just local-identify`，不会自动选串口）。
