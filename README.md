...
- 烧录：`just flash`（首次烧录会要求再次输入 `yes`，成功后尝试回填设备身份）
...
- Local USB 烧录 + 串口监视（推荐）：`just flash-monitor`
  ...
  - 常规烧录前会通过 JSONL `info` 校验 `device_id` / `mac`；全新硬件或下载模式可先 `just select-port` 再 `just flash` 做一次未识别首次烧录。两种路径都只写 app `.bin` 到 `0x10000`，不会默认写 full image。
