...
目前仓库包含 ESP32-S3 固件、Web 控制台、Tauri desktop shell，以及 released-style host tools。Local USB 的长期用户边界是 `isolapurr-devd` + `isolapurr`；Web Serial 仍是正式浏览器路径。
...
- 从源码启动 devd：`just devd-serve --bind 127.0.0.1:51200 --allow-dev-cors`
- 从源码运行 CLI：`just isolapurr devices`
