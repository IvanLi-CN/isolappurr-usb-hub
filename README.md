...
- 两版 MCU 引脚分配分别见
  [`tps-sw`](docs/mcu-pin-assignment-tps-sw.md) 和
  [`tps-fusb`](docs/mcu-pin-assignment-tps-fusb.md)，不得跨 variant 混用。
- 电源输入按 variant 区分：当前 `tps-sw` 使用 USB‑PD 输入与 DC5025 的
  既有 OR-ing 路径；设计中的 `tps-fusb` 将由 MCU 在两路 PMOS 输入之间
  执行 DC 优先的一选一控制，避免两路输入同时主动导通。
...
- `crates/isolapurr-firmware-core/`
  - 固件共享 `no_std` core crate，承载可在 host 上测试的纯逻辑，例如电源配置、显示策略、idle-bias 计算、遥测推导与 EEPROM 记录编码。
- `web/`  
...
- 维护流程真相源见 [`docs/maintainer-workflow.md`](docs/maintainer-workflow.md)；其中说明了什么时候走 user skill、developer skill、repo-private maintainer workflow、spec sync 和 project-doc sync。
...
- 刷新 Web/PWA + Desktop 安装图标：`just desktop-icons`
...
- [`skills/isolapurr-user-operations/SKILL.md`](skills/isolapurr-user-operations/SKILL.md)
- [`skills/isolapurr-developer-operations/SKILL.md`](skills/isolapurr-developer-operations/SKILL.md)
- [`skills/isolapurr-maintainer-workflow/SKILL.md`](skills/isolapurr-maintainer-workflow/SKILL.md)
...
- 当前 released surface 以 `status --device-id|--url`、`hardware save --device-id ...`、`power ...`、`diagnostics export`、`settings reset wifi|other` 为准；README 和 repo skill 不再把旧的 status/hardware-save selector 变体当成正式入口。
...
- 仓库内部维护路由见 [`docs/maintainer-workflow.md`](docs/maintainer-workflow.md)；`isolapurr-maintainer-workflow` 是 repo-private 入口，不作为普通用户安装目标宣传。
...
- 只跑共享纯逻辑测试：`just firmware-core-test`
...
- 在线且支持 `identify` capability 的设备可从侧栏 Locate 图标或 Firmware flash 的 Target 区触发 5 秒定位提示；released CLI 也支持 `isolapurr identify --device-id <device-id>` 或 `--url <base-url>`。
...
- Web verification surfaces follow
  ...
  production route proof stays on the real SPA pages, the formal owner-facing
  demo entrypoint is `?demo=true|false` on those same SPA routes, Storybook is
  reserved for reusable components and composite surfaces, and `/demo/*` pages
  plus page-level Storybook stories are not part of the allowed workflow.
