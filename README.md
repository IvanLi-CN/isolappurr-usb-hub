...
- 内容边界：使用 release 版 `isolapurr` / `isolapurr-devd` 与正式 Web Serial 路径；缺少 host tools 时，skill 会先展示安装计划并在确认后运行 release installer；不要求源码 checkout、Rust、Bun、Just 或本地构建缓存。
...
- installer 会下载匹配平台的 `isolapurr-host-tools-<platform>.tar.gz`，用同一 release 的 `SHA256SUMS` 校验，并安装 `isolapurr` / `isolapurr-devd` 到用户目录；不会自动修改 shell profile 或系统 PATH。
