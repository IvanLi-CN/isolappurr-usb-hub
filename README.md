...
目前仓库包含 ESP32-S3 固件、Web 控制台、Tauri desktop shell，以及 released-style host tools。Local USB 的长期用户边界是 `isolapurr-devd` + `isolapurr`；CLI 通过本机 IPC 连接 devd，不通过 localhost HTTP。Web Serial 仍是正式浏览器路径。
...
- 在当前项目安装用户操作 skill：
  - `npx --yes skills add . --skill isolapurr-user-operations -y`
- 在当前项目安装开发/维护 skill：
  - `npx --yes skills add . --skill isolapurr-developer-operations -y`
- 如确实需要用户级全局安装，再显式加 `--global`；默认从项目目录运行时安装为项目级 skill。
- 使用边界：
  - `isolapurr-user-operations`：面向普通用户机器，使用 release 版 `isolapurr` / `isolapurr-devd` 与正式 Web Serial 路径。
  - `isolapurr-developer-operations`：面向源码 checkout，使用 Just、源码构建、release assets、HIL/debug 与维护流程。
