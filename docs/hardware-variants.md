...
该连接要求 `SDA_TPS/SCL_TPS` 在 MCU 配置 `TPS55288` 前保持可释放状态。固件启动时先以 open-drain 释放 `SDA_TPS/SCL_TPS`，通过 `TPS55288` 的 `OE` 寄存器关闭输出并开启主动放电，延迟约 1 秒后检查总线电平；若总线被拉低，才允许短暂拉高 `CE_TPS` 做一次 hard-start。`TPS55288` 5V boot setpoint 写入成功后，固件继续释放总线并等待约 1.005 秒，再访问 `SW2303`。TPS 是否进入 5V 设定不得由 INA226 遥测判定。若 `SW2303` 上电后持续箝位 SDA，MCU 无法通过同一条 I2C 总线读取 PD 请求；固件保持 TPS boot 输出并等待总线释放，不反复掉电。
