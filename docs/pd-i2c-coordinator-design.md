...
- 固件层面 **严格限制**：I2C 只允许访问 `SW2303 (0x3C)` 与 `TPS55288 (0x74)`，禁止扫描与“误触碰其他地址”。（依据：网表 `R7=0Ω` 配置 TPS55288 地址为 `74h`）
...
- SW2303（U3）：
  - pin1  → `SDA_TPS`
  - pin16 → `SCL_TPS`
- 上拉：`RN6` 为 `SCL_TPS/SDA_TPS` 提供到 `3V3` 的 4.7 kΩ 上拉。
...
- `SW2303`：`0x3C`
- `TPS55288`：`0x74`  
  依据：网表显示 `TPS55288 MODE`（pin15）通过 `R7=0Ω` 接到 `AGND_TPS`，按数据手册 MODE 电阻配置表，对应 I2C 地址为 `74h`。
