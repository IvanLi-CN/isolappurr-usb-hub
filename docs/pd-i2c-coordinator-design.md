...
6) 5V boot setpoint 写入成功后，固件继续以 open-drain 释放 `SCL_TPS/SDA_TPS` 并等待约 3 秒，再允许任何 `SW2303` I2C 事务；等待窗口内也不得执行非必要 PD I2C 事务，包括 TPS status/fault 读取。
