...
- `SafetyAlarm`：循环播放（例如 `Tone(2700Hz, 8%, 700ms)` + `Silence(300ms)`）。
- 默认响度：低占空比（建议 5–10%），且避免长时间 100% 占空比。

### 默认频率与辨识度

- 默认基频：`2700Hz`（贴近器件标称）。
- 为提高可辨识度，可在不同 SoundId 上做轻微频率偏移（例如 2400/2700/3000Hz），但不追求复杂旋律。
...
|---|---|---:|---|---:|---|
| `BootOk` | 一次性 | 2700Hz | `短短短 + 长` | ≥2.2s | 正常开机完成 |
| `BootWarn` | 一次性 | 2400Hz | `长短长短长` | ≥2.1s | 有降级但可运行 |
| `BootFail` | 一次性 | 2000Hz | `短×6（间隔均匀）` | ≥2.2s | 初始化致命失败 |
| `WarningOnce` | 一次性 | 2400Hz | `中×5` | ≥2.1s | 非安全异常 |
| `ErrorOnce` | 一次性 | 2700Hz | `快×6` | ≥2.2s | 一般错误 |
| `SafetyAlarm` | 持续 | 2700Hz | `700ms on / 300ms off` 循环 | 持续 | 安全风险（需持续提示） |
| `RecoverOnce` | 一次性（预留） | 3000Hz | `短短 + 长` | ≥2.0s | 异常恢复提示（本次可不接线） |
