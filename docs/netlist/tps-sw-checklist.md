...
- [x] `LED/MODE(LEDD)` 外围（tps-sw 已确认）：
  ...
  - 固件约束：MCU 必须以高阻输入读取（禁用内部上下拉/禁止输出驱动），避免反向干扰 `LEDD` 的模式/指示逻辑；固件将 `LEDD=0` 作为 active-low 有效样本并用饱和积分器输出稳定的上游链路指示。
...
- [x] `PU_CED` 由 `U1(CH318T) IO2` 驱动，并通过 CH318T 边带映射来自下位机侧 `U2.IO2=PU_CE`；`PU_CE` 接 MCU `GPIO36`。固件默认把 `GPIO36` 拉低保持上游数据连通，只有编译期恢复开关启用时才启动期短暂拉高断开。
