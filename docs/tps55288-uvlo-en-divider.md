...
| 项目 | 数值 |
| --- | --- |
| 分压电阻 `R1`（`VIN -> EN/UVLO`） | **200 kΩ** |
| 分压电阻 `R2`（`EN/UVLO -> GND`） | **36 kΩ** |
| UVLO 启动点 `V_IN(UVLO_ON)`（typ，按 Eq.1 估算） | **≈ 8.063 V** |
| UVLO 关断点 `V_IN(UVLO_OFF)`（typ，按 Eq.1/2 估算） | **≈ 7.063 V** |
| UVLO 迟滞 `ΔV_IN(UVLO)`（typ，按 Eq.2 估算） | **≈ 1.00 V** |
| 数据手册关键参数 | `V_UVLO = 1.20/1.23/1.26 V`，`I_UVLO = 4.5/5/5.5 µA` |
...
### 5.1 不要把 MCU GPIO 直接接到 EN/UVLO 分压节点

这个分压网络的设计目标是让 `EN/UVLO` 在 **8 V** 左右达到 **≈1.23 V**（UVLO 阈值），因此它在高 `VIN` 下的节点电压会随 `VIN` 成比例升高：

- 以当前阻值 `R1=200k`、`R2=36k`：`VIN=24V` 时 **纯分压**已到 `V_EN≈3.66V`
- 还要叠加 `I_UVLO`（typ 5 µA）对 `R2` 的抬升：`ΔV≈I_UVLO·R2≈0.18V`（typ）

因此在 `VIN≈24V` 的工况下，`EN/UVLO` 节点电压可能到 **≈3.8 V** 量级。

以 **ESP32‑S3** 为例，其 datasheet 的 Absolute Maximum Ratings（Table 5‑1）里，**Allowed input voltage 最大值为 3.6 V**。这意味着：

- 把 `EN/UVLO` 分压节点直接接到 `GPIOxx`（包括 `GPIO37`）存在**过压/可靠性风险**
- 如果你确实需要“让 MCU 控制 TPS55288 上电/关断”，请用 **晶体管隔离**，让 GPIO 只驱动晶体管的栅/基极，而不是直连该高压分压节点

工程上更稳的做法（常用两种）：

1. **低边下拉关断**：在 `EN/UVLO` 节点加一个 NMOS（漏接 `EN/UVLO`、源接 GND），需要关断时把节点拉到 `<0.4V`（TPS55288 shutdown threshold），平时让分压正常工作（代价是关断时分压仍有 `VIN→GND` 泄放电流）。
2. **按数据手册 Figure 7‑2 断开分压泄放**：用 NMOS 把 `VIN` 与 `R1` 串接，并满足“EN logic high > threshold + Vth(Q1)”的门限裕量要求（注意 3.3 V GPIO 下对 NMOS `VGS(th)` 的约束会更苛刻）。
