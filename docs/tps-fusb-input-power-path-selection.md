...
`VIN_DC` 通过单颗 `200kOhm` 上臂和 `20kOhm` 下臂进入
`GPIO1/ADC1_CH0`，ADC 节点对地并联 `100nF`：

```text
VIN_DC -> 200k -> VIN_DC_SENSE -> 20k -> GND
                    |
                  100nF
                    |
                   GND
...
- 项目有效输入范围：9 V 至 28 V，28 V 为额定最高输入。
- 分压比：1:11。
- 9 V -> 约 0.818 V。
- 24 V -> 约 2.182 V。
- 28 V -> 约 2.545 V。
- 28 V 时上臂电阻压降约 25.45 V，功耗约 3.24 mW。最终选定的
  单颗 `200kOhm` 电阻必须满足工作电压、脉冲耐受、功耗和容差要求。
- ADC 使用 11 dB 衰减配置；不要求 ADC 线性测量超过
  28 V 的输入。
- 分压器戴维宁等效电阻约 18.18 kOhm，与 `100nF` 的时间常数约
  1.82 ms。ADC 初始化或输入连接后等待至少 10 ms 再使用采样结果。
...
- 两颗输入 PMOS、VGS clamp、Gate-Source 上拉和 gate driver 按 28 V 额定
  最高输入、浪涌、电流、SOA、热和压降选型；器件绝对最大值不得当作额定工作点。
...
- [ ] ADC 在 9 V、24 V 和 28 V 完成校准、容差和噪声测试。
