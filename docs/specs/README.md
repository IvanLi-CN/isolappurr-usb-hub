...
| dzcaw | USB-C TPS Power Config | 已完成 | `dzcaw-usb-c-tps-power-config/SPEC.md` | 2026-06-17 | SW2303-only persisted power config plus runtime `Power`/TPS `DISCHG` controls, manual high-voltage heating warning, and TPS `IOUT_LIMIT` diagnostics/readback semantics |
...
| tvhca | USB-C 下行通道路由切换 | 已完成 | `tvhca-usb-c-downstream-route/SPEC.md` | 2026-05-18 | MCU / USB-C route switching, EEPROM persistence, versioned port capability declarations, firmware source metadata, HTTP/USB JSONL API, Web UI, and dual-button shortcut |
...
| x6cua | Device identify and locate | 已验证 | `x6cua-device-identify/SPEC.md` | 2026-08-03 | Fixed five-second device identity display, buzzer, explicit capability negotiation, and Web Locate controls |
...
| tfzd3 | PWA 启动壳与启动恢复 | 已完成 | `tfzd3-pwa-launch-recovery/SPEC.md` | 2026-07-22 | Installed-PWA startup shell, failure recovery shell, proactive prompt-preserving update discovery, GitHub Pages hashed-asset retention window, and `/flash` PWA workbench metadata caching plus in-session refresh |
| m7q4v | tps-fusb 双 FUSB302B PD 与电源路径硬件 | 待实现 | `m7q4v-tps-fusb-dual-pd-hardware/SPEC.md` | 2026-08-29 | Next hardware variant; archived independent netlist baseline, with PCB, production data, and firmware still pending |
| — | tps-fusb firmware 与硬件发现 | 部分完成（2/6） | `tps-fusb-firmware-hardware-discovery/SPEC.md` | 2026-08-29 | Shared admission, topology-only compatibility, safe mismatch mode, and multi-surface flash pre/postflight are implemented; FUSB Source HIL and release gates remain |
