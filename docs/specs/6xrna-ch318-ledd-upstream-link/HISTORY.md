...
- 2026-05-16：根据 HUSB305-01 数据手册复核，`STAT` 原始配置为 LLD indication，HUSB305-02 才是 fault indication；产品 API/UI 将 `UP0_PG/GPIO18` 调整为 `hub.isolated_usb_fault`，高电平表示故障。

## 关键决策

- `LEDD` 只作为 active-low 状态源使用，不承诺 USB 枚举完成、速率或协议细节。
- `UP0_PG` 作为 active-high 隔离侧 USB 故障指示源使用；HUSB305-01 原始 `STAT` 语义为 LLD indication。
