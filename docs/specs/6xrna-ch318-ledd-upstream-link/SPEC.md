...
- 设备需要判断隔离侧 USB 故障指示，以及隔离侧 USB 链路是否 ready。
- `HUSB305-01` 数据手册定义 `STAT` 为开漏输出；HUSB305-01 用于 LLD indication，负载电流高于 `I_LLD` 时下拉，负载电流低于阈值时回到高阻。HUSB305-02 才把 `STAT` 配置为 fault status indication。
- `HUSB305-01 STAT(UP0_PG)` 已接到 MCU `GPIO18`，外部 `100kΩ` 上拉到 `3V3`，本产品 API/UI 将该节点作为隔离侧 USB 故障指示，高电平表示故障。
...
- 用 `GPIO18/UP0_PG` 提供 Hub 级 `isolated_usb_fault` 输入，并保持 active-high 语义：高电平表示隔离侧 USB 故障。
...
- 固件每秒读取一次 `UP0_PG`，并以 `raw_high` 直接更新 `hub.isolated_usb_fault`。
...
- `UP0_PG` 采样必须以 `raw_high` 作为 active-high 故障样本。
...
- `hub.isolated_usb_fault` 必须直接反映最近一次 `UP0_PG` 采样结果。
...
- `GET /api/v1/ports` 与 USB JSONL `ports.get` 继续返回 `hub.upstream_connected: bool`，并新增 `hub.isolated_usb_fault: bool` 与 `hub.isolated_usb_ready: bool`。
- `hub.upstream_connected` 是兼容字段，值必须等于 `hub.isolated_usb_ready`。
- `hub.isolated_usb_fault` 字段语义：来自 `HUSB305-01 STAT(UP0_PG)` 的 active-high 低频采样结果，表示隔离侧 USB 故障指示。
- `hub.isolated_downstream_connected` 仅作为旧开发中间字段保留兼容，不再作为 Web Dashboard 的显示依据。
...
- Given `UP0_PG=High`
  When 经过一次 1 秒采样周期
  Then `hub.isolated_usb_fault=true`。

- Given `UP0_PG=Low`
  When 经过一次 1 秒采样周期
  Then `hub.isolated_usb_fault=false`。
...
- [x] M5: 新增 `UP0_PG -> GPIO18` 采样路径与隔离侧故障状态字段
...
- 假设：本产品将 `UP0_PG=1` 作为隔离侧 USB 故障指示；HUSB305-01 数据手册原始 `STAT` 语义为 LLD indication，不是 HUSB305-02 fault indication。
...
- HUSB305 data sheet, Hynetek Semiconductor: HUSB305-01 `STAT` = LLD Indication; HUSB305-02 `STAT` = Fault Indication.
- `src/bin/main.rs`
