# 隔离侧 USB 状态指示实现（#6xrna）

## 当前实现状态

- 固件已将 `GPIO6/LEDD` 配置为无内部上下拉的高阻输入，并通过 `hub.upstream_connected` 对外暴露状态。
- 固件已将 `GPIO18/UP0_PG` 配置为无内部上下拉的高阻输入，并通过 `hub.isolated_usb_fault` 对外暴露隔离侧 USB 故障指示。
- LEDD 采样使用 active-low 普通 GPIO 读取，采样周期为 `1000ms`。
- UP0_PG 采样使用 active-high 普通 GPIO 读取，采样周期为 `1000ms`。
...
- HTTP API 与 USB JSONL `ports.get` 新增 `hub.isolated_usb_fault` 与 `hub.isolated_usb_ready`，Web Dashboard 优先显示这两个隔离侧语义字段。
...
- 需要实机确认 `UP0_PG=High` 与 `hub.isolated_usb_fault=true` 在当前硬件短接路径下保持一致。
