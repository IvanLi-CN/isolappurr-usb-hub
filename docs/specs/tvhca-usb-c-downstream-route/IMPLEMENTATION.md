...
- Capability contract：固件核心共享序列化 `capability_schema=1`、`power_set`、`data_replug`、`data_set` 和 firmware build metadata；HTTP、USB JSONL 与主机桥沿用同一声明。Web resolver 将每项能力解析为 `supported`、`unsupported` 或 `unknown`，旧设备保持安全禁用但不再被称为需要升级。
- Firmware provenance：HTTP 与 USB JSONL `info` 响应公开 `firmware.build.source_sha`（完整 Git 提交）和 JSON boolean `dirty`；Web 概览把现有 `VITE_BUILD_SHA` 标为 `Console build`，设备信息面板单独展示 firmware source SHA。
- Duplicate state surface：端口卡和 USB-C 电源侧栏移除独立 `State` 行/图标，`Power` / `Data link` 按钮保留最近确认状态图标和 `aria-pressed`；`replugging` 使用 warning/busy 外观，并优先提示 `Data path is switching. Wait for it to finish.`。
