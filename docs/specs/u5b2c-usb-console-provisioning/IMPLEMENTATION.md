...
- Add device discovery: a successful add marks one result as added and keeps the dialog open while other merged discovery results remain available. The latest completed browser IP scan is retained for ten minutes with live/demo storage isolation; desktop scan progress is run-scoped and remains separate from live service discovery.
