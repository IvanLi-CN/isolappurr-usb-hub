...
- Shared device-info copy: the shell derives the current resolved name, complete lowercase canonical `device_id`, and online active transport at render time; `DeviceHeaderName` writes one-line comma-separated `text/plain` and `text/markdown` payloads through a testable clipboard helper, falling back to `navigator.clipboard.writeText` when multi-MIME clipboard support is unavailable. Desktop and mobile AppLayout stories use the same payload, and unit/interaction coverage verifies formatting, MIME values, fallback, and copy feedback.
- Add device discovery: a successful add marks one result as added and keeps the dialog open while other merged discovery results remain available. The latest completed browser IP scan is retained for ten minutes with live/demo storage isolation; a completed sweep with at least one HTTP response is retained even when other probes are offline or browser-blocked, while an all-blocked sweep remains non-persistable. Restored scan state also restores its normalized CIDR input; desktop scan progress is run-scoped and remains separate from live service discovery.
- Desktop discovery scan ownership: start requests carry a client request token in addition to the monotonic server `runId`; cancellation prefers the run ID, falls back to token ownership while the start response is pending, and keeps the no-argument path only for confirmed legacy responses. Desktop CIDR validation rejects ranges larger than the web host limit before host enumeration.
