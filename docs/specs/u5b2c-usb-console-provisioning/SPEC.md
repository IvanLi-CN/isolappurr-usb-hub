...
- Development reset MUST return the reset method and evidence that the serial port remained available or re-enumerated. Development monitor MUST keep the serial port open until interrupted and classify boot, JSONL, panic/backtrace, ordinary log lines, and raw binary/control-byte frames. JSON monitor output MUST NOT coerce non-UTF-8 or control-byte frames into ordinary `log` or `jsonl` records.
