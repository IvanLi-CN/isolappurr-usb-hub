...
- `isolapurr-devd` exposes the new device-centric daemon API, token bootstrap, Local USB scanning, leases, session traces, storage import/list/save, Wi-Fi/ports/status/route/diagnostics proxy methods, firmware catalog validation, and guarded flash/reset endpoints.
- `isolapurr` exposes released-style CLI entrypoints for hardware memory, discovery/devices/status, Wi-Fi, ports, flash, reset, monitor, and diagnostics.
