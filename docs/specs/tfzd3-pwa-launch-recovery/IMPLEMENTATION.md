...
- `web/public/boot-shell.js` 接管启动期错误、挂载超时、service worker 更新检查、`waiting` worker 激活、自愈 reload 和缓存修复动作；即使浏览器没有把安装窗口报告为 standalone，也会保留失败观察器和 Repair 动作，避免旧 PWA shell 继续白屏。
...
- 2026-07-22 的线上 Chrome 安装态复核确认：Chrome 安装窗口可能不匹配 `display-mode: standalone`，导致旧 boot shell 没有挂载失败观察器；当前实现改为健康时隐藏、失败时仍显示恢复壳，并在没有 `waiting` worker 时先主动 `registration.update()`。
