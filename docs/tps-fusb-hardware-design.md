...
网表中 LED 的阴极汇合到 `LED_TPS`，阳极侧分别经 `R8=2.7kOhm` 和
`R25=680Ohm` 接到 `3V3`，所以 GPIO47 应作为低端开漏吸电流输出使用：
GPIO47=Low 时连接到该网络的视觉组同步点亮，GPIO47=High 时释放为高阻并
保持 LED 关闭。这个拓扑不需要 `LED_TPS` 逻辑上拉，也不得启用 GPIO47 的
内部上拉或下拉。

按 `Vf=0` 的保守电阻上限估算，3.3V 时 GPIO47 的总灌电流不超过
`3.3/2.7k + 3.3/680 = 6.07mA`，3.6V 时不超过约 6.62mA；实际值由
LED 正向压降决定。该电流是所有并联 LED 分支之和，限流电阻不保证并联
LED 之间均流。推挽高电平虽然在正常状态下也能实现熄灭，但会主动驱动共阴极
节点，开漏释放态对复位、掉电和未来外部节点变化更安全。
...
| 47 | `LED_TPS` | 开漏输出；低有效的同步 LED 组控制，High/Hi-Z=关 |
