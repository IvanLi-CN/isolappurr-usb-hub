...
- “正常界面”以两列展示两口的实时 V/A/W，并允许 USB-C 顶部带最小 mode / badge 语义提示（无菜单、无交互）。
- 通过**绝对位置**（左/右两列）对应屏幕下方的 USB‑A / USB‑C 高压 / 复用输出通道两接口。
- 采用**定宽字符格**，避免闪烁与布局跳动。
...
- 字形源：6×8
- 放大比例：`SX=3`, `SY=4` → 字形像素为 `18×32`，在 tile 内居中（上/下留白边）
- 颜色：背景恒为白；前景色按 `docs/specs/j9twf-gc9307-normal-ui/SPEC.md` 的“需求（Requirements）/MUST/颜色”（1bpp：每个字符 tile 仅有前景/背景两色）

> 以上参数用于生成“与固件同渲染方式”的 1:1 像素效果图；实现阶段如更改渲染策略或配色，需要同步更新本文档的效果图。

### 3.1 USB-C 顶部语义

- 自动跟随场景：左 badge 继续显示 `PD` / `PPS` / `DC`，右 badge 可显示目标电压或 `OFF`
- 手动 TPS 输出且 `output_enabled=true`：左 badge 显示手动设定电压，格式为 `x.xxV`
- 手动 `usb_c_path_mode=force`：右 badge 固定显示 `FOCUS`
- 其它手动路径：右 badge 仅显示真实导通状态 `ON` / `OFF`
- USB-C 的 `V/A/W` 始终显示 U17 INA226 实测值，不显示手动目标值或 SW2303 请求值
