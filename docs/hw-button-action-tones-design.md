...
- **安全优先**：当安全报警（`SafetyAlarm`）持续播放时，动作结果提示音必须抑制（不播放、不排队、不延后补播）。
...
- 不新增音量/静音配置项。
- 不改变现有 Boot/Warning/Error/Safety 既定提示音模式（除新增动作提示音外）。
...
3) **安全报警期间抑制**
- 系统进入安全报警（`SafetyAlarm` 循环） → 用户按左右键 → 动作仍可执行（是否执行取决于后续业务接入），但动作结果提示音必须直接丢弃，不排队、不延后补播。
...
- 动作结果提示音必须 **drop**（不播放、不入队）。
- 原因：避免报警解除后“补播旧结果音”造成误导。

落点建议：

- 在 `PromptToneManager.notify(...)` 对“动作结果事件”做显式分支：若 `safety_active==true` 则直接返回。
- 由提示音层统一实施抑制，调用方不需要理解内部状态。
...
- `SafetyAlarm` active 时按键触发的动作结果提示音被抑制：不播放、也不会在报警结束后补播。
