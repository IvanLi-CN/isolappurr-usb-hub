...
- 在 `INT_TPS` 已实现并验证可用后，关闭 1Hz 周期性 `diag:` 打印（避免刷屏与掩盖关键信息）。

### Non-goals

- 不调整 TPS55288 的配置（包含但不限于 ILIM/OVP/SR/OCP_DELAY 等寄存器）。
...
- 固件：在 `INT_TPS` 已实现并验证可用后，关闭 1Hz `diag:` 打印（以及其中对 TPS `STATUS` 的周期性读取）。
