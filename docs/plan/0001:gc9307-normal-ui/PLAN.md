...
- 硬件映射（tps-sw）：
  ...
  - 地址兼容策略（现场异常兼容件）：
    - U13：优先 `0x40`；仅当主地址返回 `Address NAK` 时，允许回退到 `0x44`。
    - U17：优先 `0x41`；仅当主地址返回 `Address NAK` 时，允许回退到 `0x45`。
    - 两口地址解析彼此独立；单口失败不得阻断另一口继续解析与采样。
    - 若某口在启动阶段仍未解析成功，后续刷新周期继续重试主地址解析，并在 `Address NAK` 时允许回退。
...
- 风险：兼容地址 fallback 仅对 `Address NAK` 生效；其它总线错误仍按测量失败处理，避免把瞬时通信异常误判为“应切换到兼容地址”。
- 开放问题：无。
