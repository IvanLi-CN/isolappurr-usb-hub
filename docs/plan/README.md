...
- `待合并`：验收通过但尚未创建 PR，或尚未进入合并流程。
- `待合并（#<pr_id>）`：已创建 PR，等待合并（PR 尚未 merge）。
- `已完成`：PR 已合并完成并关闭。
...
- 仅在以下情况更新 `Last`（不要因为改措辞/排版就更新）：
  - `Status` 变化（含写入 PR 号、从 `待验收` → `待合并` 等）
  - `PLAN.md` 的里程碑勾选变化
  - 范围/验收标准冻结或发生实质变更
...
|-----:|-------|--------|------|------|-------|
| 0001 | GC9307 正常界面规范（USB‑A + USB‑C/PD 双口电参量） | 待合并（#12） | `0001:gc9307-normal-ui/PLAN.md` | 2026-01-08 | - |
