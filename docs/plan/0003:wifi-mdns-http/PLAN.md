...
- 主流系统尽量可用；当 `.local`/mDNS 不可用时，提供本机屏幕显示 IPv4 作为兜底：
  - 左右键同时按住 1–3 秒后松手，界面显示 ID/IP 信息（>3 秒作废；未联网显示 `NO WIFI`/`NO IP` 等可测试提示）。
...
- UI/交互（最小）：
  - 左右键同时按住 `1–3s` 后松手 → 显示网络信息页/overlay（ID、IPv4；可选网关/DNS）。
...
- UI：
  - 双键同时长按 `1–3s` 后松手触发一次“显示网络信息”动作；`>3s` 作废；显示内容可读且可测试。
...
- [x] M5: （impl）UI：双键同时长按 1–3 秒后松手显示网络信息（>3 秒作废）；未联网时给出明确提示
...
- 左右键同时按住 1–3 秒后松手触发一次显示动作（`>3s` 作废）：
