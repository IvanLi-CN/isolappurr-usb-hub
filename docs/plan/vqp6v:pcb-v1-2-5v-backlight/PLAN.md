# PCB v1.2：背光 MOS 极性修复 + TPS55288 外部 +5V 供电（tps-sw）（#vqp6v）
...
- PCB v1.2 计划把 `TPS55288` 改为外部 `+5V` 供电，需要在实现前冻结：
...
- `SW2303` 供电关系按网表记录：`VIN=VOUT_TPS`，与 TPS 输出建立顺序强相关。
...
- 变更点（以网表证据为准）：
  - 背光：`Q8(BSS84)` 源漏极性修复
  - 供电：`U14(TPS55288)` 外部 `+5V`；`U16(SW2303)` 的 `VIN` 接 `VOUT_TPS`
...
- **SW2303 供电关系（网表可证）**：`U16.VIN=VOUT_TPS`，`U16.VDD` 仅连接本地去耦网络。
- **I2C 电平域（网表/手册可证）**：
...
- `CE_TPS` 会影响 `TPS55288` 输出，因此会影响由 `VOUT_TPS` 供电的 `SW2303`。
- 若 `SW2303` 在 `VOUT_TPS` 低于可工作电压时箝住 `SDA_TPS`，MCU 将无法通过同一条 I2C 总线先访问 `TPS55288`。
...
- 已明确 v1.2 下 `CE_TPS` 的影响范围：影响 `TPS55288 EN/UVLO`，并会通过 `VOUT_TPS` 影响 `SW2303` 供电。
...
- `docs/netlist/tps-sw-checklist.md`: 增加 v1.2 供电、启动依赖与 `RMODE`/地址策略说明
- `docs/pd-i2c-coordinator-design.md`: 明确 v1.2 下 `CE_TPS` 的影响范围与 TPS/SW 启动顺序
...
- [ ] M2: 复核关键网络（背光 MOS、TPS55288 VCC/MODE、SW2303 VIN、隔离域）
- [ ] M3: 同步更新受影响文档并在 `docs/hardware-variants.md` 更新 sha256

## 方案概述（Approach, high-level）

- 以网表连通性为“最小可验证证据”，冻结：背光开关极性、TPS55288 外部 VCC 与 `RMODE` 策略、SW2303 供电关系。
- `TPS55288` I2C 电平可由其数据手册门限佐证；`SW2303` 的 I2C 电平兼容性必须以其手册为准后再冻结。

## 风险 / 开放问题 / 假设（Risks, Open Questions, Assumptions）

- 风险：
  - `SW2303` 在 `VOUT_TPS` 未建立时可能影响 `SDA_TPS/SCL_TPS` 总线释放，导致 TPS I2C boot 配置无法开始。
  - `CE_TPS` 行为变化可能影响“断电/重插”类功能预期，因为它会通过 TPS 输出影响 PD 芯片工作电压。
- 需要决策的问题：
