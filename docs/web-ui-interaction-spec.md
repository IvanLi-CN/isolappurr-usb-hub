...
- 已安装 PWA 冷启动必须先显示独立于 React bundle 的品牌启动壳；启动壳只对 standalone / installed 会话强制开启，普通浏览器页不要求同样首屏体验，也不得因普通挂载延迟触发 standalone 启动超时，但显式启动失败观察器仍必须挂载，失败时不得白屏。
- 启动壳必须负责监听入口脚本 `404`、同源脚本错误、未处理 promise rejection 与 standalone 挂载超时；出现故障时先自动恢复，再落到失败壳。
- 失败壳必须提供 `Try again` 与 `Repair app` 两个动作；`Repair app` 只能重置 service worker 与静态缓存，不得清空保存设备、主题或其他 owner 本地数据。
- `vite-plugin-pwa` 负责 manifest、service worker、Workbox 预缓存与离线 App shell。
- App shell 必须维护统一的 PWA install/display-mode runtime：拦截 `beforeinstallprompt` 以驱动自有 Install CTA，监听 `appinstalled` 与 `display-mode`/`window-controls-overlay` 状态变化，并向根布局暴露稳定的 installed shell data contract。
- 当浏览器提供原生安装提示且当前尚未安装时，全局 header 右侧必须显示 Install CTA；`/about` 页面必须始终保留安装说明，并在没有原生提示时给出浏览器菜单 fallback 指引。
- 更新策略使用 prompt 模式：健康会话在 service worker 注册完成、页面回到前台、网络恢复和 60 分钟轮询时主动检查 `sw.js` / `registration.update()`；发现新 service worker 后显示 toast，用户点击更新时先关闭 toast，再调用 `updateSW(true)` 刷新到新版本；只有启动故障恢复路径允许主动 `registration.update()`、自动激活 `waiting` worker 并 reload。
- 当 owner 在健康会话中对某一候选更新点击 `Later` 时，同一标签页会话内不得重复弹出该候选更新 toast；下次启动应自然进入新版本。
...
- 面向安装面的 icon URL（favicon、Apple touch、manifest `icons[*].src`）必须携带基于图标内容的版本后缀；当图标素材变更时，Chrome/macOS PWA 安装态必须看到新的 URL，避免长期复用旧缓存。
- PWA manifest 必须暴露 `/` 与 `/flash` 两个 install shortcuts，并维护来自稳定 owner-facing app shell 状态的 manifest screenshots；截图资产落在 `web/public/pwa/`，不得用营销海报或社交预览图替代。
- installed shell 仅做 minimal app-shell 适配：根布局通过 `data-display-mode` 与 `data-window-controls-overlay` 暴露状态，header/main 需要消费 safe-area/titlebar inset，但不得借此重做导航或页面信息架构。
- GitHub Pages 发布产物必须保留当前版本与 retention 窗口内的旧 hash 静态资源；窗口规则为“最近两版或 14 天，以较晚到达者为准”。stable 发布必须优先从既有 GitHub Release web-dist 资产恢复旧 hash；没有凭证或尚无匹配 web-dist 资产的 bootstrap 路径才退回当前 live Pages manifest 或 `sw.js`，并通过显式 retention 清单控制保留与淘汰。
