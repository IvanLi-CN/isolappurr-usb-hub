...
- 已安装 PWA 冷启动必须先显示独立于 React bundle 的品牌启动壳；启动壳只对 standalone / installed 会话强制开启，普通浏览器页不要求同样首屏体验，但启动失败观察器仍必须挂载，失败时不得白屏。
- 启动壳必须负责监听入口脚本 `404`、同源脚本错误、未处理 promise rejection 与挂载超时；出现故障时先自动恢复，再落到失败壳。
