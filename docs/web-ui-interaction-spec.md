...
- GitHub Pages 发布产物必须保留当前版本与 retention 窗口内的旧 hash 静态资源；窗口规则为“最近两版或 14 天，以较晚到达者为准”。stable 发布必须优先从既有 GitHub Release web-dist 资产恢复旧 hash，只有本地/bootstrap 路径才退回当前 live Pages manifest 或 `sw.js`，并通过显式 retention 清单控制保留与淘汰。
