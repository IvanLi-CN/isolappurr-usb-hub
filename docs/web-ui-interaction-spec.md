...
- 已安装 PWA 冷启动必须先显示独立于 React bundle 的品牌启动壳；启动壳只对 standalone / installed 会话强制开启，普通浏览器页不要求同样首屏体验。
...
- 更新策略使用 prompt 模式：健康会话在 service worker 注册完成、页面回到前台、网络恢复和 60 分钟轮询时主动检查 `sw.js` / `registration.update()`；发现新 service worker 后显示 toast，用户点击更新时先关闭 toast，再调用 `updateSW(true)` 刷新到新版本；只有启动故障恢复路径允许自动激活 `waiting` worker 并 reload。
