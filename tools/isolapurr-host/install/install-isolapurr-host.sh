...
REPO_URL="https://github.com/IvanLi-CN/isolappurr-usb-hub"
VERSION="latest"
...
ARCHIVE="isolapurr-host-tools-${SLUG}.tar.gz"
...
printf 'IsolaPurr host tools install plan\n'
printf '  source: %s\n' "$BASE_URL"
printf '  archive: %s\n' "$ARCHIVE"
...
installed_version=""
installed_isolapurr=""
if [ -x "${INSTALL_DIR}/isolapurr" ]; then
  installed_isolapurr="${INSTALL_DIR}/isolapurr"
elif command -v isolapurr >/dev/null 2>&1; then
  installed_isolapurr="$(command -v isolapurr)"
fi
if [ -n "$installed_isolapurr" ]; then
  installed_version="$("$installed_isolapurr" --version 2>/dev/null | awk '{print $NF}' || true)"
fi
devd_available=0
installed_devd=""
if [ -x "${INSTALL_DIR}/isolapurr-devd" ]; then
  installed_devd="${INSTALL_DIR}/isolapurr-devd"
elif command -v isolapurr-devd >/dev/null 2>&1; then
  installed_devd="$(command -v isolapurr-devd)"
fi
...
if [ -n "$installed_version" ] && [ -n "$target_version" ]; then
  ...
  if [ "$cmp" -eq 0 ] && [ "$FORCE" -ne 1 ] && [ "$devd_available" -eq 1 ]; then
    printf 'isolapurr %s is already installed; use --force to reinstall\n' "$installed_version"
    exit 0
  fi
  if [ "$cmp" -lt 0 ] && [ "$FORCE" -ne 1 ]; then
    die "refusing to downgrade isolapurr ${installed_version} to ${target_tag}; use --force to override"
  fi
fi
...
[ -f "${extract_dir}/isolapurr" ] || die "archive missing isolapurr"
[ -f "${extract_dir}/isolapurr-devd" ] || die "archive missing isolapurr-devd"

mkdir -p "$INSTALL_DIR"
install -m 0755 "${extract_dir}/isolapurr" "${INSTALL_DIR}/isolapurr"
install -m 0755 "${extract_dir}/isolapurr-devd" "${INSTALL_DIR}/isolapurr-devd"

"${INSTALL_DIR}/isolapurr" --help >/dev/null
"${INSTALL_DIR}/isolapurr-devd" --help >/dev/null

printf 'installed IsolaPurr host tools to %s\n' "$INSTALL_DIR"
case ":${PATH}:" in
  *":${INSTALL_DIR}:"*) ;;
  *)
    printf 'PATH note: add this directory before using isolapurr from a new shell:\n'
