...
- `bun run retain-pages-assets` builds `dist/asset-retention.json` and copies forward supported old hashed assets. In stable release jobs it prefers existing GitHub Release web-dist assets via `GITHUB_REPOSITORY` + `GITHUB_TOKEN`; local/bootstrap runs without those credentials fall back to the live Pages site.
