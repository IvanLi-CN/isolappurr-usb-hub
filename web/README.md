...
- Brand assets: `bun run brand-assets` regenerates light and dark product posters, the cropped standalone product render, the uncropped approved product render archive, the transparent product cutout export, light and dark social previews, and the PNG logo from the source images in `src/assets/brand/`
...
- Manifest shortcuts are fixed to the Dashboard (`/`) and Firmware flash workbench (`/flash/`); stable Pages builds generate a concrete `/flash/` SPA shell in addition to the generic `404.html` fallback for dynamic routes.
...
- `web/src/assets/brand/product-poster-source.png`, `web/src/assets/brand/product-poster-dark-source.png`, `web/src/assets/brand/product-render-source.png`, `web/src/assets/brand/product-render-full-source.png`, `web/src/assets/brand/product-render-cutout-source.png`, `web/src/assets/brand/github-social-preview-source.png`, and `web/src/assets/brand/github-social-preview-dark-source.png` are the source marketing images. `bun run brand-assets` exports the light poster at `3072x3840`, copies the approved native dark poster at `1122x1402` without image processing, exports light and dark public social previews at `1280x640`, retains the bright GitHub preview `.github/social-preview.png`, and exports the existing product render outputs.
...
- `bun run test:icons` validates the geometry contract and marketing image dimensions: regular icons keep padding, maskable icons stay edge-aligned, desktop PNG assets are not transparent placeholders, light/dark poster and social preview exports keep their exact required sizes, and the public dark poster remains byte-identical to its approved generated source.
